...
def get_routing_table(netmiko_connection: object, vrfs=None):
    ...
    print(netmiko_connection, type(netmiko_connection))
    if not vrfs:
...
class RoutingIos:
    ...
    def database(self):
        ...
        if self.route_age:
            try:
                DataBaseOps.db_update_ios_xe(self.db, self.cursor, vrf=self.vrf, prefix=self.prefix, protocol=self.protocol,
